...
function wslib.new(fd, addr, header, handler, conf, url)
    local conf = conf or {}
...
function wslib:recv_frame()
    ...
    local ok, data, err = pcall(socket.read, self.fd, 2)
    if not ok then
        return false, nil, "socket is closed."
    end
    if not data then
