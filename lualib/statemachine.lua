...
function StateMachine:setupState(cfg)
    ...
    if type(cfg.initial) == "string" then
        ...
        self.initial_ = utils.clone(cfg.initial)
    end
...
function StateMachine:_setState_(_state)
    self.current_ = _state
end
...
function StateMachine:onError_(event, error, message)
    log.log("%s [StateMachine] ERROR: error %s, event %s, from %s to %s", tostring(self.target_), tostring(error), event.name, event.from, event.to)
    log.log("###"..message)
end
