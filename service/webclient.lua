...
-- @usage skynet.call(webclient, "lua", "request", "http://www.host.com")
-- @usage skynet.send(webclient, "lua", "request", "http://www.host.com", nil, nil, true)
local function request(url, get, post, no_reply, headers)
    ...
    if headers and next(headers) then
        for _, str in pairs(headers) do
            webclient:set_httpheader(req, str)
        end
    end
    if not req then
        return skynet.ret()
...
skynet.start(function()
    skynet.dispatch("lua", function(session, source, command, ...)
        if command == "start" then
            local args = {...}
            skynet.ret(skynet.pack(skynet.register(args[1])))
        else
            request(...)
        end
    end)
